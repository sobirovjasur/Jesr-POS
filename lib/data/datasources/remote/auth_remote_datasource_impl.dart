...
class AuthRemoteDataSourceImpl implements AuthDataSource {
  ...
  Future<Result<UserModel>> signInWithGoogle() async {
    try {
      ...
      var googleSignInAccount = await googleSignIn.attemptLightweightAuthentication();

      if (googleSignInAccount == null && googleSignIn.supportsAuthenticate()) {
        googleSignInAccount = await googleSignIn.authenticate(scopeHint: Constants.authScopes);
      }

      if (googleSignInAccount == null) {
        return Result.failure(error: 'Google sign-in was cancelled.');
      }
